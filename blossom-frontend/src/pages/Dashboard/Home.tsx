...
function getPhase(day: number) {
  if (day <= 5)  return { name: 'Menstrual Phase',   key: 'menstrual'  as const }
  if (day <= 13) return { name: 'Follicular Phase',  key: 'follicular' as const }
  if (day === 14) return { name: 'Ovulation',        key: 'ovulation'  as const }
  return              { name: 'Luteal Phase',        key: 'luteal'     as const }
}

// Per-theme insight copy
const THEME_INSIGHTS = {
  ...
  rapunzel: [
    ...
    { icon: '🌺', type: 'Nutrition',      title: 'Paint with colour',    text: 'Nourish your body with vibrant foods. Berries, seeds and leafy greens this phase.' },
  ...
  cinderella: [
    { icon: '✨', type: 'Cycle insight',  title: 'Peak clarity ahead',   text: 'The clock is on your side. Your focus and poise are at their finest this week.' },
...
const RING_LEGEND = {
  frozen:     ['Menstrual', 'Follicular', 'Ovulation', 'Luteal'],
  tiana:      ['Menstrual', 'Follicular', 'Ovulation', 'Luteal'],
  rapunzel:   ['Menstrual', 'Follicular', 'Ovulation', 'Luteal'],
  cinderella: ['Menstrual', 'Follicular', 'Ovulation', 'Luteal'],
}
...
// Hardcoded for now — replaced with real API data in Backend Part 3
const CYCLE_DAY   = 12
const TOTAL_DAYS  = 28
const DAYS_LEFT   = TOTAL_DAYS - CYCLE_DAY

export default function Home() {
  const { themeId } = useThemeStore()
  const { user }    = useAuthStore()
  const theme       = THEMES[themeId]
  const phase       = getPhase(CYCLE_DAY)
  ...
  return (
    <div>
      {/* ---- HERO ---- */}
      <div style={{ padding: '44px 24px 24px', position: 'relative', overflow: 'hidden' }}>
        {/* Decorative orbs */}
        <motion.div
        ...
        <div style={{
          fontFamily: 'var(--font-head)', fontSize: 36, fontWeight: 400,
          color: 'var(--acc2)', lineHeight: 1.1, marginBottom: 20,
        ...
        <div style={{
          ...
          {theme.phasePoetry[phase.key].replace(/"/g, '')}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ width: 240, height: 240, position: 'relative', marginBottom: 20 }}>
          <canvas
            ref={canvasRef}
            width={240}
            height={240}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
          {RING_LEGEND[themeId].map((label, i) => (
      ...
      <div style={{ padding: '0 24px 28px' }}>
        <motion.button
          ...
          style={{
            ...
            background: logged ? 'var(--text-hint)' : 'var(--acc3)',
            border: 'none',
            color: logged ? 'var(--text-muted)' : 'var(--text)',
            fontFamily: 'var(--font-head)', fontSize: 15,
            fontWeight: 400, cursor: 'pointer',
            letterSpacing: 0.5, transition: 'background 0.4s, color 0.4s',
          }}
        >
          {logged ? 'Logged successfully ✦' : 'Track today'}
        </motion.button>
      </div>
